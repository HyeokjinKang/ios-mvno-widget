...
`CONFIG.LINE_INDEXES`로 표시할 회선을 인덱스 배열로 고를 수 있다. 0부터 시작하고 적은 순서
그대로 표시되므로(`[2, 0]`이면 세 번째가 위, 첫 번째가 아래) 순서도 함께 정할 수 있다.
`null`이나 빈 배열이면 전체를 서버가 준 순서대로 보여준다. 위젯을 여러 개 놓고 각각 다른
인덱스를 지정하면 회선을 나눠 볼 수 있다.
